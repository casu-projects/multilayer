...
// CoolSync 스냅샷 백로그 근본 수정 (Fix A - v2: Postfix 구조)
...
// v2: v1의 Prefix 선전진이 베이스 ACK 처리를 "not newer"로 스킵시켜
...
// 고속 깜빡임/스턱온 - 실측). v2는 Prefix에서 ACK ID만 캡처하고 Postfix에서
// 베이스가 처리한 객체(이미 ackId)를 제외한 나머지만 전진한다 - 베이스 패킷 갱신 보존
// 진단(임시): ACK 도착 로그(플레이어별 10초 스로틀) - ACK 미도착 시 별도 원인(클라 측)
[HarmonyPatch]
internal static class CoolSyncAckBaselinePatch
{
    ...
    // Prefix에서 캡처한 ACK ID - Postfix에서 소비 (시스템 인스턴스별)
    ...
    // ACK ID 캡처 - 원본이 다시 읽도록 위치 복원 (무간섭)
    ...
    // 베이스 ACK 처리(스냅샷 내 객체의 패킷/기준선 갱신) 이후,
    // 그 외 객체(베이스가 처리하지 않은 쿨 객체)의 기준선 ID만 전진
