...
/// <summary>도착/동결(마이그레이션) 플레이어의 forcedelete 큐에서 **본인 netId**를 제거 (2026-08-07).
...
/// 본인 바디는 연결 중 삭제될 일이 없으므로 억제가 안전하다. 다른 객체의 forcedelete는 유지.</summary>
[HarmonyPatch]
internal static class ForcedeleteOwnNetIdSuppressPatch
{
    ...
    /// <summary>Server_ResetDeltaOnObj — 클라이언트의 객체 정보 요청 처리 (본인 netId 요청이
    /// objstate 부재로 forcedelete가 되는 경로).</summary>
    ...
    /// <summary>Server_ReceiveAck — ACK 스냅샷 처리에서 서버 객체가 없으면 forcedelete가 되는 경로.</summary>
