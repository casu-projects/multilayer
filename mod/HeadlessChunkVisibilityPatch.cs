...
/// <summary>헤드리스 서버의 청크 활성화를 고정된 호스트의 카메라가 아니라 실제 플레이 중인 유저 기준으로 바꿈.
/// 유저가 해당 청크를 지나가도 잠시 동안 유지해서 액체, 떨어지는 아이템이나 상자, AI가 멈추는 현상을 줄인다.</summary>
[HarmonyPatch]
...
/// <summary>게임 원본 기준으로는 월드 밖에 주차된 서버 카메라를 기준으로 청크를 다시 꺼버리지만
/// 데디케이트 서버에서만 그 방식을 건너뛰고 플레이중인 유저 기준으로 갱신을 사용하게 만듬.</summary>
[HarmonyPatch]
