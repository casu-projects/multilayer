...
/// <summary>퇴장 시 바닐라 DestroyCharacterIfNoLocal의 인벤토리 드랍 방지 (데디서버 전용).
/// 바닐라는 퇴장 정리에서 Server_DropAllInventory()를 호출해 인벤토리를 바닥에 드랍하는데,
/// 인벤토리는 NetPlayer_OnDestroy Prefix(SubmitPlayer)가 이미 오케스트레이터에 제출한 상태라
/// 드랍이 발생하면 재접속 복원 시 같은 아이템이 바닥과 인벤토리에 중복 존재하게 된다.
/// 따라서 드랍만 생략하고 나머지 바디 정리는 동일하게 수행한다.</summary>
