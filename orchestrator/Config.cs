...
public sealed class OrchestratorConfig
{
 ...
 // 최대 동시 접속 플레이어 수 - 초과 시 게이트웨이가 접속 시도 단계에서 거부
 // (AUTH_INFO로 게이트웨이에 전달, 오케스트레이터도 SESSION_CONNECTED에서 2차 검증).
    public int MaxPlayers { get; set; } = 32;

 // 디버그 로그 표시 여부 - false면 게이트웨이/오케스트레이터/에이전트/모드의
 // 디버그급 로그가 숨겨진다. `verbose on/off` 명령으로 런타임 토글 + json 영속 반영.
    public bool Verbose { get; set; } = false;

 // 락다운(lockdown) 중에도 접속 허용할 SteamID64 목록 (orchestrator.json
 // 운영자가 편집한 뒤 lockdown 명령 실행 시 재로드된다).
    public List<ulong> LockdownBypass { get; set; } = new();
 // 마이그레이션 트랜잭션 단계별 타임아웃(초). 월드젠(~15초) + 클라이언트 재생성 여유.
 ...
 // 게임 레이어 수 - 수동 마이그레이션(`migrate`)의 "다음 레이어" 계산과
 // 목적지 유효 범위 검증에 사용 (모드가 LAYER_END로 보고하는 값과 일치해야 한다).
    public int MaxLayers { get; set; } = 5;

 // 프리웜 레이어 목록 - 에이전트 연결 시 이 레이어들을 수용량 내 최대한
 ...
 // 밴 목록 파일 - pwd 기준 고정 이름 (config 미노출).
    [JsonIgnore]
