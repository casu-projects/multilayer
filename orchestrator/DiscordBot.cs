...
// Discord 관리 봇 - 단일 채널(DiscordChannelId)에 접속/퇴장·마이그레이션·관리자 호출·
// 사망/리스폰·채팅을 알림하고, 채널 메시지를 게임 채팅으로 전달한다 (speaker="관리자")
// 토큰/채널 미설정 시 비활성. 아바타/프로필 링크는 steamId != 0일 때만 동작한다
internal sealed class DiscordBot
{
    ...
    // Ready 전이 시 콜백 (채널 주제 초기 동기화 등). 생성 후 설정 - Ready는
    // StartAsync 이후 비동기로 발화하므로 생성 전 참조 문제가 없다
    ...
    // 전체 콘솔 로그 라인 접수 (TimestampedConsoleWriter.OnConsoleLine - 접두사 포함)
    // 빈 줄 제외, 큐 오버플로 시 오래된 라인 폐기 + 생략 카운터 누적
    ...
    // 콘솔 로그 배치 전송 루프 - 1초 주기로 큐를 비워 최대 1900자 단위 메시지로
    // 콘솔 채널에 전송한다 (연결 전 라인은 큐에 쌓였다가 연결 후 일괄 전송)
    ...
    // 접속/퇴장 알림 - Steam 아바타 + 프로필 링크 (steamId ≠ 0일 때만)
    ...
    // 서버 시작/종료 알림 (메시지 채널) - "서버가 켜졌습니다."/"서버가 꺼졌습니다."
    ...
    // 레이어 이동(마이그레이션 커밋) 알림 - L{from} > L{to}
    ...
    // 관리자 호출 알림 - DiscordAdminUserId가 설정된 경우 멘션
    // 설정 ID가 길드 역할이면 역할 멘션(&lt;@&amp;id&gt;), 아니면 사용자 멘션(&lt;@id&gt;) - 자동 감지
    // (Discord ID는 전역 유일 네임스페이스라 모호성 없음, 역할은 인텐트와 무관하게 항상 캐시)
    ...
    // 관리자 멘션 문자열 생성 - 역할 ID면 &lt;@&amp;id&gt;, 사용자 ID면 &lt;@id&gt;
    ...
    // 채널 주제(description) 갱신 - 접속 인원 실시간 표시
    // 중복 방지: 현재 주제와 같으면 스킵 (수동 수정은 1회 유지)
    ...
    // 봇 자체 로그 - 표준 콘솔 규격 "[HH:mm:ss orch:bot] ..."으로 출력하고
    // 릴레이 라인으로도 수집되므로 콘솔 채널 로그 릴레이에 자연히 포함된다
    ...
    // Discord 채팅 -> 게임 - 서버 닉네임 해석 후 릴레이 (닉네임 REST 폴백 포함)
    ...
    // 서버 닉네임 해석 - SocketGuildUser.Nickname 우선, 없으면 REST 조회 폴백
    // 봇에 GUILD_MEMBERS 인텐트가 없어도 REST(GetUserAsync)는 닉네임을 반환한다
