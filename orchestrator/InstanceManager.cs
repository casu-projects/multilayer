...
public sealed class InstanceInfo
{
    ...
    // 마지막 RESET 이후 플레이어가 접속했었는지 — Prewarm 유휴 재초기화를 점유 사이클당
    // 1회로 제한 (플레이어 없이 무한 월드젠 루프 방지).
    public bool HasPlayedSinceReset { get; set; }
}

// 인스턴스 수명주기 — 노드 에이전트 경유 스폰/정지, READY 검증. 모든 접근은 메인 스레드 전용.
public sealed class InstanceManager
{
    ...
    // 해당 depth의 인스턴스를 보장 (없으면 SPAWN). Stopped/Crashed는 재스폰한다 —
    // 유휴 정지 후 재접속 시 고아 주소로 라우팅되는 것 방지. preferredMachine: 프리웜
    // 경로의 선호 에이전트 (등록+여유 시 우선, 아니면 알고리즘).
    ...
    private InstanceInfo? Spawn(int depth, string? preferredMachine = null)
    {
        ...
        _instances[key] = info;
        _runNeeded.Add(key); // 모든 인스턴스는 모드 연결 시 START_RUN 필요.
    ...
    // preferred가 등록되고 여유가 있으면 우선, 아니면 수용량 대비 사용량이 가장 낮은 에이전트.
    ...
    // 프리웜 전용 선호 에이전트 — `prewarm set <agent>`로 지정 (미연결/포화 시 알고리즘 폴백).
    // 수요 스폰에는 적용되지 않는다. pwd/prewarm-agent.json에 영속화.
    ...
    // Prewarm 인스턴스 레이어 초기화 — 모드에 RESET을 발신해 ToMainMenu → 재월드젠 후
    // READY 재보고를 유도한다 (프로세스 생존 — 부팅 비용 제거). 실패 시 크래시 처리.
    ...
    // PrewarmLayer를 수용량 내 최대한 스폰 (에이전트 등록마다 재실행 — 누락분 자동 보충).
    // 동시 부팅 리소스 경쟁을 피해 큐에 넣고 TickPrewarm이 순차 처리한다.
    public void PrewarmLayers()
    {
        if (_config.PrewarmLayer.Length == 0) return;
        foreach (int depth in _config.PrewarmLayer)
        {
    ...
    // Prewarm 수동 정지 → 재시작 예약 (실제 종료 시점에 _prewarmRestartAt으로 이전).
    ...
    // Prewarm 인스턴스 정지 후 자동 재시작 예약 — 실제 종료 시점부터 delay 후 재스폰.
    public void SchedulePrewarmRestart(string key, TimeSpan delay)
        => _prewarmRestartRequested[key] = delay;

    // depth가 PrewarmLayer 소속인지 — 자동 재시작 대상 판정.
    public bool IsPrewarmDepth(int depth) => _config.PrewarmLayer.Contains(depth);

    // 순차 프리웜 진행 — 현재 항목이 READY가 되면 다음을 스폰. 실패 시 현재 항목을 버린다.
    public void TickPrewarm()
    {
        ...
        while (_prewarmQueue.Count > 0)
        {
            ...
            // 스폰 불가 (에이전트 없음/수용량 초과) — 이 항목은 스킵.
        }
    }

    // 런 시작 대기 표시 — 스폰/MOD_HELLO 시점에 표시되고, MOD_HELLO에서 바로 START_RUN을
    // 전송한다. 인스턴스 수명당 1회 전송.
    ...
    {
        if (key == null) return;
        // 이미 실행 중(READY/IDLE)인 인스턴스는 런 시작 불필요.
        InstanceInfo? info = Find(key);
    ...
    public void OnModHello(ControlHub.ClientConnection conn, ModHelloPayload payload)
    {
        ...
        // START_RUN은 모드 연결 시점에 발신 — 월드젠이 플레이어 없이 진행된다. 모드 측
        // HandleStartRun이 이미 생성된 세계면 무시하므로 재수용/재시작에도 안전.
        _runNeeded.Add(key);
        TrySendStartRun(key);
    }

    // 인스턴스 READY 전이 (Booting/Starting → Ready). 실제 전이된 경우에만 true — 중복
    // INSTANCE_READY 보고 시 호출부가 ROUTE-ON-READY 푸시를 1회만 수행하게 한다.
    ...
    public void OnInstanceExited(string key, int code)
    {
        ...
        // Prewarm 수동 정지 → 실제 종료 시점부터 유예 후 자동 재시작 예약.
    ...
    // 유휴 정리 (O2 인스턴스 수명주기 + Prewarm 유휴 재초기화).
    ...
    public void TickIdle(DateTime now, Func<string, int> connectedCountForInstance)
    {
        ...
        {
            // Starting/Booting/IdleReset은 유휴 판정 대상 아님.
            ...
            bool isPrewarm = _config.PrewarmLayer.Contains(info.Depth);
