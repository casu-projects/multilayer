...
public sealed class InstanceInfo
{
    ...
    /// <summary>마지막 레이어 초기화(RESET) 이후 플레이어가 접속했었는지 — Prewarm 인스턴스의
    /// 유휴 재초기화를 "점유 사이클당 1회"로 제한 (플레이어 없이 무한 월드젠 루프 방지).</summary>
...
public sealed class InstanceManager
{
    ...
    /// preferredMachine: 프리웜 경로의 선호 에이전트 (등록+여유 시 우선, 아니면 알고리즘).</summary>
    ...
    /// <summary>에이전트 선택 — preferred가 등록되고 여유(Used &lt; Capacity)가 있으면 그것을
    /// 우선하고, 아니면 수용량 대비 사용량이 가장 낮은 에이전트로 폴백한다.</summary>
    ...
    /// <summary>프리웜 전용 선호 에이전트 — `prewarm set <agent>`로 지정 (미연결/포화 시
    /// SelectMachine이 알고리즘으로 폴백). 수요 스폰(마이그레이션/접속)에는 적용되지 않는다.
    /// pwd/prewarm-agent.json에 영속화 — 재시작 후에도 유지된다 (2026-08-03).</summary>
    ...
    /// 실패(모드 미연결/ack 타임아웃) 시 Crashed 처리 — 다음 EnsureInstance가 재스폰 폴백.</summary>
    ...
    /// 재실행 — 누락분 자동 보충, 기존 인스턴스는 재스폰 없음). 동시 부팅 리소스 경쟁으로
    /// 월드젠이 실패하는 것을 방지하기 위해 큐에 넣고 TickPrewarm이 순차 처리한다 —
    /// 한 인스턴스의 READY(월드젠 완료)를 기다린 뒤 다음을 스폰한다 (2026-08-03 실측:
    /// 3개 동시 스폰 시 1번만 월드젠 성공).</summary>
    ...
    /// <summary>Prewarm 인스턴스 수동 정지 → 재시작 예약 (OnInstanceExited가 실제 종료 시점에
    /// _prewarmRestartAt으로 이전). 키: 인스턴스 키.</summary>
    ...
    /// <summary>Prewarm 인스턴스 정지 후 자동 재시작 예약 — 실제 종료(OnInstanceExited) 시점부터
    /// delay 후 재스폰된다 (instance stop의 유지 관리 용도).</summary>
    public void SchedulePrewarmRestart(string key, TimeSpan delay)
        => _prewarmRestartRequested[key] = delay;

    /// <summary>depth가 PrewarmLayer 소속인지 — instance spawn 금지/자동 재시작 대상 판정.</summary>
    public bool IsPrewarmDepth(int depth) => _config.PrewarmLayer.Contains(depth);

    /// <summary>순차 프리웜 진행 (메인 루프 틱) — 현재 항목이 READY(월드젠 완료)가 되면
    /// 다음을 스폰한다. 실패(스폰 불가/크래시) 시 현재 항목을 버리고 다음으로 넘어간다
    /// (누락분은 다음 에이전트 등록의 PrewarmLayers가 재보충).</summary>
