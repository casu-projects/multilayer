...
public sealed class InstanceManager
{
 ...
 // 프리웜 - PrewarmLayer의 레이어를 수용량 내 최대한 스폰 (에이전트 등록마다
 // 재실행 - 누락분 자동 보충, 기존 인스턴스는 재스폰 없음). 동시 부팅 리소스 경쟁으로
 // 월드젠이 실패하는 것을 방지하기 위해 큐에 넣고 TickPrewarm이 순차 처리한다
 // 한 인스턴스의 READY(월드젠 완료)를 기다린 뒤 다음을 스폰한다 (실측:
 // 3개 동시 스폰 시 1번만 월드젠 성공).
 ...
 // depth가 PrewarmLayer 소속인지 - instance spawn 금지/자동 재시작 대상 판정.
