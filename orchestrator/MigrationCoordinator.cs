...
/// <summary>마이그레이션 트랜잭션 (O6-4, O3) — FREEZE → READY 대기 → SWAP → RESUME → WORLDGEN → COMMIT.
/// 단일 기록자 규칙 enforcement + 단계별 타임아웃/롤백 + epoch 멱등 + WAL 영속 (P2).
...
public sealed class MigrationCoordinator
{
    ...
    private sealed class Transaction
    {
        ...
        /// <summary>SWAP 인플라이트 (2026-08-02): 발신~ack 수신 사이 Tick이 SWAP을 재발신해
        /// 게이트웨이가 백엔드를 이중 연결("이름 중복" 추방)하는 것을 차단한다.</summary>
        public bool SwapSent { get; set; }
        /// <summary>WAL 복구된 트랜잭션 — 스왑 후 상태면 RESUME 재발신이 필요할 수 있다.</summary>
        public bool Recovered { get; set; }
        /// <summary>복구 후 RESUME 재발신 완료 여부.</summary>
        public bool ResumeSent { get; set; }
        /// <summary>리스폰 트랜잭션 — FREEZE 캡처 스킵 + RESUME payload null + COMMIT 시 세이브 폐기.
        /// WAL에 영속되어 크래시 복구 후에도 프레시 신규 의미를 유지한다.</summary>
    ...
    /// <summary>이 인스턴스를 목적지로 하는 진행 중 마이그레이션 수 (유휴 정리 계수용 —
    /// 마이그레이션 도착 대기 중인 목적지를 유휴로 오판정해 강제 정지하지 않도록 한다).</summary>
    ...
    /// <summary>복구 시 스왑 후 트랜잭션은 세션을 목적지로 고정 (재접속 라우팅 정합).</summary>
    ...
    private void PersistWal()
    {
        try
        {
            // System.Text.Json은 struct를 딕셔너리 키로 직렬화할 수 없다 (2026-08-02 실측:
    ...
    // ── 진입: RESPAWN (!respawn — P5) ──

    /// <summary>리스폰 처리 — 완전 신규 취급 (세이브 폐기 + 세션 프레시화).
    ...
    /// 스킵 + RESUME payload null → 목적지에서 프레시 생성 + 보급품 지급.</summary>
    ...
    /// <summary>목적지 READY 확인 후 SWAP 발행 (P1 READY 게이트). 인플라이트(SwapSent)면
    /// 재발신하지 않는다. 미READY면 Tick이 재시도.</summary>
    ...
    /// <summary>게이트웨이 BACKEND_CONNECTED — 스왑 완료 시점 (목적지가 월드 준비 후 접속 성공).</summary>
    ...
    /// <summary>RESUME 전송 (데이터는 보류분/디스크, ghostClientIds 재산출). ack 수령 시 WAL 전이.</summary>
    private void SendResume(Transaction tx)
    {
        ...
        // 2026-08-02 수정: 같은 목적지로 동시 마이그레이션 중인 플레이어는 제외한다 —
    ...
    /// <summary>마이그레이션 중 플레이어 이탈 (구 인스턴스의 NetPlayer.OnDestroy 보고) —
    /// 보류 데이터 확정 + 세션 Depth 증가 (S7). Freezing(스왑 전) 상태에서만 이탈로
    /// 처리하며 그 이후는 구 인스턴스 정리 부산물로 무시한다 (실퇴장은
    /// SESSION_DISCONNECTED → OnPlayerQuitDuringMigration이 항상 동시에 처리).</summary>
    ...
    /// <summary>마이그레이션 중 플레이어 퇴장 (게이트웨이 SESSION_DISCONNECTED — 실퇴장).</summary>
    ...
    /// <summary>이탈 확정: 데이터는 디스크가 정본(제출 시 기록됨), 위치는 복원하지 않도록
    /// 세션 Depth만 toDepth로 증가시켜 영속화한다 (재접속 시 새 레이어 배정 +
    /// 레이어 불일치로 위치 스킵).
    /// 2026-08-02 수정: 출발지·목적지 모드에 RELEASE를 전송해 동결을 해제한다 — 이전에는
    /// 이탈 확정 시 모드가 동결 상태로 남아, 재접속한 플레이어가 조용화 타깃 게이트
    /// (IsFrozenTarget)에 막혀 다른 플레이어 바디·월드 객체·복원 인벤토리가 전혀
    /// 동기화되지 않는 버그가 있었다 (데이터는 유실이 아니라 동기화 차단).</summary>
    ...
    /// (ROUTE-ON-READY 연동: Tick 폴링 대신 READY 이벤트 기반).</summary>
    ...
    /// 구버전 모드/게이트웨이와의 하위 호환으로 수용한다 (멱등 규칙은 오케스트레이터가 원본).</summary>
    private static bool IsCurrent(Transaction tx, int? epoch) =>
        !epoch.HasValue || epoch.Value == tx.Epoch;

    /// <summary>다른 플레이어가 같은 목적지로 마이그레이션 진행 중인지 (고스트 제외 판정).</summary>
