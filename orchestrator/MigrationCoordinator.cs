...
// 단일 기록자 규칙 + 단계별 타임아웃/롤백 + epoch 멱등 + WAL 영속. 모든 접근은 메인 스레드 전용.
public sealed class MigrationCoordinator
{
    private enum TxState
    {
        Idle,
        Freezing,
        WaitingReady,   // 목적지 READY 확인 후에만 SWAP 발행
        Swapping,
        Resuming,
        Worldgen,
    }
    ...
    private sealed class Transaction
    {
        ...
        // SWAP 인플라이트 — 발신~ack 사이 Tick이 SWAP을 재발신해 게이트웨이가 백엔드를
        // 이중 연결("이름 중복" 추방)하는 것을 차단한다.
    ...
    // 마이그레이션 커밋 완료 이벤트 (Discord 알림 등 외부 구독용).
    ...
    // 이 인스턴스를 목적지로 하는 진행 중 마이그레이션 수 — 유휴 정리 계수용 (목적지를
    // 유휴로 오판정해 강제 정지하지 않도록).
    ...
    private void PersistWal()
    {
        try
        {
            // System.Text.Json은 struct를 딕셔너리 키로 직렬화할 수 없다 — string 키로 기록.
    ...
    // 수동 마이그레이션 (`migrate <player> [targetLayer]`) — LAYER_END와 동일한 전체
    // 트랜잭션. 실패 시 콘솔 피드백용 사유 문자열을 반환한다 (null = 성공).
    ...
    // fromDepth 1: 인플레이스 무로딩 리스폰 — 세이브 폐기 + 세션 프레시만 (트랜잭션 없음).
    // fromDepth N: 레이어 1 하향 마이그레이션 트랜잭션 (IsRespawn 플래그).
    ...
    // 목적지 READY 확인 후 SWAP 발행. 인플라이트(SwapSent)면 재발신하지 않는다.
    ...
    private void SendResume(Transaction tx)
    {
        ...
        // 같은 목적지로 동시 마이그레이션 중인 플레이어는 제외한다 — 포함하면 도착자
        // 클라이언트가 함께 도착하는 유저의 NetPlayer를 파괴해 "서로가 안 보이는" 버그가
        // 발생한다 (Session.InstanceId는 COMMIT 전까지 출발지를 가리키므로 Migrating
        // 상태만으로는 구분 불가).
    ...
    public void OnResumeDone(PlayerKey player, int? epoch)
    {
        ...
        // 여기서 재전송하면 클라이언트가 재생성을 취소/재시작해 영구 대기할 수 있다.
        // WORLDGEN_DONE만 대기.
    }
    ...
    // Freezing(스왑 전) 상태에서만 이탈로 처리한다. 실퇴장은 SESSION_DISCONNECTED가
    // OnPlayerQuitDuringMigration으로 항상 동시에 처리한다.
    ...
    // 이탈 확정: 데이터는 디스크가 정본, 세션은 목적지로 배치해 재접속 시 새 레이어로
    // 라우팅한다. 출발지·목적지 모드에 RELEASE를 전송해 동결을 해제한다.
    ...
    private void Abort(Transaction tx, string reason)
    {
        ...
        // post-swap(월드젠 타임아웃 등) → 목적지, 그 외 → 출발지 (물리적 연결 인스턴스).
    ...
    // epoch 검증 — 메시지의 epoch가 현재 트랜잭션과 일치하는지. epoch 부재 시 수용 (구버전 호환).
