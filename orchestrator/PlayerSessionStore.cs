...
// 플레이어 세션/라우팅 원본 — LayerRouter + ClientIdRegistry 통합. 게이트웨이에
// TABLE_SNAPSHOT/ROUTE_UPDATE 전송. 모든 접근은 메인 스레드 전용.
public sealed class PlayerSessionStore
{
    ...
    // SESSION_CONNECTED — 상태 확정 + 인스턴스 배정. 웜 인스턴스(READY/IDLE)면 즉시 라우팅,
    // 콜드 인스턴스면 INSTANCE_READY 수신 시 PushRoutesForInstance가 라우팅한다.
    ...
    // 인스턴스 READY 전환 시 해당 인스턴스에 배정된 대기 세션을 일괄 라우팅.
    ...
    // 인스턴스 종료 — 해당 인스턴스에 배정된 Connecting(백엔드 미연결) 세션을 재배정한다.
    ...
    // 종료 시 전 세션 Offline 영속화 — 재시작 시 클린 상태. 마이그레이션 중이던 세션도
    // Offline으로 기록되지만 WAL 복구가 목적지/상태를 재설정한다.
    ...
    // 리스폰: 플레이어를 완전 신규 접속자 상태로 리셋 (depth=1, 라우팅 해제).
    // keepOnline: 레이어 1 인플레이스 리스폰은 라우팅을 보존한다 — 해제하면 이후 LAYER_END가
    ...
    // READY/IDLE 인스턴스의 라우트만 미러 — 부팅/월드젠 중 인스턴스는 제외 (게이트웨이가
    // 월드젠 거절 폴링을 하지 않도록).
